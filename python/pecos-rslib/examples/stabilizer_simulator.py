...
def run_stabilizer_specific_circuit() -> None:
    ...
    # Run the circuit multiple times
    num_shots = 100
    print(f"Running stabilizer circuit for {num_shots} shots...")
    all_results = simulator.run_circuit_with_shots(szz_circuit, num_shots)
    ...
    for shot_results in all_results:
        results_dict = {result_id: outcome for result_id, outcome in shot_results}
        outcome_str = "".join(str(results_dict.get(i, "X")) for i in range(2))
        outcome_counts[outcome_str] += 1
    ...
    print("\n==== End of Stabilizer-Specific Circuit Example ====")


if __name__ == "__main__":
    run_bell_state_experiment()
    run_ghz_state_experiment()
    run_stabilizer_specific_circuit()
